...
class BSMPricer:
    ...
    def _bsm_price_from_vol(self, S_o: float, K: float,
        ...
        opt_price = cp_flag * (S_o * discount_d * cdf(cp_flag * d1)
                                - K * discount_r * cdf(cp_flag * d2))  # type: ignore
    ...
    def _volatility_from_market_price(self, S_o: float, K: float,
        ...
        implied_vol = brentq(objective_function, 1e-6, 2.0, 
    ...
    def greeks(self) -> Dict[str, float]:
        ...
        theta = (
            ...
            - self.cp_flag * self.risk_free_rate * self.instrument.strike_price \
                * discount_r * cdf_cp_d2 + self.cp_flag * self.dividend_yield \
                    * self.spot_price * discount_d * cdf_cp_d1  # type: ignore
        ) if T > 0 else 0.0

        rho = self.cp_flag * self.instrument.strike_price \
            * T * discount_r * cdf_cp_d2 if T > 0 else 0.0  # type: ignore
